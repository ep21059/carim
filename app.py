...
def get_scene_id(filename):
    """
    NuScenesのファイル名からScene IDを抽出します。
    形式: n008-2018-05-21-11-06-59-0400__CAM_FRONT__1526915243012465.jpg
...
def get_timestamp(filename):
    """
    NuScenesのファイル名からタイムスタンプを抽出します。
    形式: ...__1526915243012465.jpg
    """
    basename = os.path.basename(filename)
    parts = basename.split("__")
    if len(parts) >= 3:
...
def main():
    ...
    st.caption("コンテキスト認識型 検索システム (Context-Aware Retrieval)")
    ...
    query = st.text_input("検索条件", placeholder="例: A pedestrian crossing the street at night (英語で入力してください)")
